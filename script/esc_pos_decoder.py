...
class EscPosDecoder:
    ...
    def __init__(self, verbose=0):
        ...
        self.decoder_tree = {
            EscPosDecoder.ESC: {
                b"*": self._set_bit_image_mode,        # 0x2A
            ...
            EscPosDecoder.GS: {                             # 0x1D
                b"*": self._define_downloaded_bit_image,    # 0x2A
                ...
                b"v": {
                    b"0": self._print_raster_bit_image
                }
            }
        }
    ...
    def _print_raster_bit_image(self, m, xL, xH, yL, yH):

        data = self.DATA_FUCKER

        image_start_idx = data.find(b'\x1D\x76\x30')

        # print(escpos_data)
        escpos_data = data[image_start_idx:]

        width = int.from_bytes(escpos_data[4:6], 'little') * 8
        height = int.from_bytes(escpos_data[6:8], 'little')
        ...
        for byte_index in range(image_data_start, len(escpos_data)):
            byte = escpos_data[byte_index]
            for bit in range(8):
        ...
        im.save("logo.png".format(width, height))
        
        self.log_info(f"Print raster bit image")
    ...
    def _set_bit_image_mode(self, mode):
        image_mode = int(mode[0])
        self.log_info(f"Set bit image mode : {image_mode}")

    def _define_downloaded_bit_image(self, tt):
        self.log_info(f"Define downloaded bit image : {tt}")
    ...
    def _cut_paper(self, cut_mode):
        ...
        cut_mode = int(cut_mode[0])
        self.log_info(
            f"Cut with cut mode: {cut_mode}/0x{cut_mode:02X} (depending on printer)")
    ...
    def _process_graphics_data(self, pl, ph, m, fn, a, bx, by, c, xL, xH, yL, yH, *data):

        data_length = int.from_bytes(pl + ph, byteorder='big')
        if fn[0] == 50:
            self.log_info(
                f"Print the graphics data in the print buffer. \n Processing graphics data of length {data_length}, mode {m[0]}, function {fn[0]}")
        elif fn[0] == 112:
            print(pl, ph, m, fn, a, bx, by, c, xL, xH, yL, yH)
            self.log_info(
                f"Store the graphics data in the print buffer (raster format). \n Processing graphics data of length {data_length}, mode {m[0]}, function {fn[0]}")
        else:
            print(pl, ph, m, fn)
            self.log_info(
                f"Processing graphics data of length {data_length}, mode {m[0]}, function {fn[0]}")
        pass
    ...
    @decoder_method
    def _data_buf_to_output(self, print_buff=False):
        if self.data_buf:
    ...
    def printer_method_args_wrapper(self, func, args):
        self.log_info(
            f"Calling printer method '{func.__qualname__}' with args={self.cmd_arg_buf}")
        try:
            func(*args)
        except:  # TODO/FIXME: catch only specific exceptions
            # fail silently and only output a warning (productive mode)
            self.log_warn(
                f"Printer method '{func.__qualname__}' failed to execute.")
        self.cmd_buf = []
        self.cmd_arg_buf = []
        self.cmd_expected_nargs = 0
        self.decoder_mode = EscPosDecoder.EscPosDecoderMode.FILL_DATA_BUF
    ...
    def feed_bytes(self, bytes):
        ...
        """
        self.DATA_FUCKER = bytes
        for b in bytes:
            # decoder need a bytes-like object, not ints
            b = b.to_bytes(1, byteorder)
            self._feed_byte(b)
    ...
    def _feed_byte(self, b):
        ...
        if self.decoder_mode == EscPosDecoder.EscPosDecoderMode.FILL_DATA_BUF:
            ...
            if len(self.cmd_arg_buf) == self.cmd_expected_nargs:
                ...
                self.printer_method_args_wrapper(
                    printer_method, self.cmd_arg_buf)

    @decoder_method
    def _find_printer_method(self):
        # copy the dict to be searched as it is modified during the process
        search = self.decoder_tree
        search_depth = 0
        try:
    ...
    def _process_command_buffer(self):
        ...
        if callable(printer_method):
            ...
            if nargs == 0:
                self.log_info(
                    f"Calling printer method '{printer_method.__qualname__}' without arguments")
                ...
                self.log_debug(
                    f"Start collecting arguments for printer method '{printer_method.__qualname__}'")
